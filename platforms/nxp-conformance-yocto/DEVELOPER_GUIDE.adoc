...
IMPORTANT: This guide was imported from the original `geisa/lee-mockup`
repository. The project now lives under
`platforms/nxp-conformance-yocto/` in the GEISA Community repository.
Use `README.md` for current project scope and source-setup instructions.
The remaining build and flashing details are retained from the original
project and have not been fully revalidated during migration.
...
Clone the GEISA Community repository:

  $ git clone https://github.com/geisa/community.git
  $ cd community/platforms/nxp-conformance-yocto

This project keeps its Yocto dependencies local to the project instead of
registering them in the Community repository's root `.gitmodules`.

Initialize the exact imported dependency revisions with:

  $ ./scripts/setup-sources.sh

To inspect the dependency URLs and revisions without cloning them:

  $ ./scripts/setup-sources.sh --dry-run

NOTE: The initial build process can take several hours and consume substantial
disk space.
