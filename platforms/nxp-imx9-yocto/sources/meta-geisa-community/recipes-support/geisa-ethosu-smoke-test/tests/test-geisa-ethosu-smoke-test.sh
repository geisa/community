...
grep -F 'GEISA Ethos-U smoke test' "$workdir/output"
grep -F 'PASS: Ethos-U accelerated inference verified' "$workdir/output"
...
grep -F 'PASS: Ethos-U accelerated inference verified' "$workdir/profile-output"
