...
def python_soabi(sysroot: Path, site: Path) -> str:
    dynload = sysroot / site.parent / "lib-dynload"
    values = set()
    for path in dynload.glob("*.cpython-*.so"):
        match = re.search(r"\.(cpython-[^.]+)\.so$", path.name)
        if match:
            values.add(match.group(1))
    if len(values) != 1:
        raise ValueError(f"expected one CPython SOABI in {dynload}, found: {sorted(values)}")
    return values.pop()
...
def assemble(sysroot: Path, wheel: Path, output: Path, source_revision: str) -> None:
    ...
    soabi = python_soabi(sysroot, site)
...
def verify(root: Path) -> None:
    ...
    if document.get("architecture") != "aarch64" or not re.fullmatch(r"cpython-[0-9]+-[a-z0-9_]+-linux-gnu", str(document.get("python-soabi", ""))):
