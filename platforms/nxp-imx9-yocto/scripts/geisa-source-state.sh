...
outer_generated_excludes=(
    ":(exclude)${prefix}build-*"
)

outer_source_excludes=()
for source_id in "${release_profile_source_ids[@]}"; do
    outer_source_excludes+=(":(exclude)${prefix}$(release_profile_source_path "$source_id")")
done
...
outer_excludes=("${outer_source_excludes[@]}" "${outer_generated_excludes[@]}")
