#!/usr/bin/env bash
# SPDX-License-Identifier: Apache-2.0
# Copyright (C) 2026 PragSol Consulting, LLC
# Website: https://www.pragsolconsulting.com/
#
set -euo pipefail

platform_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
export GEISA_PLATFORM_ROOT="${platform_root}"
# shellcheck source=release-profile.sh
. "${platform_root}/scripts/release-profile.sh"
release_profile_select "${GEISA_RELEASE:-}"

if [ "$#" -eq 1 ]; then
    machine="${MACHINE_IMX93}"
    build_id="$1"
elif [ "$#" -eq 2 ]; then
    machine="$1"
    build_id="$2"
else
    echo "usage: $0 [geisa-imx93|geisa-imx95] <build-id>" >&2
    exit 2
fi
case "${machine}" in
    "${MACHINE_IMX93}"|"${MACHINE_IMX95}") ;;
    *) echo "unsupported machine: ${machine}" >&2; exit 2 ;;
esac
deploy_dir="${platform_root}/${BUILD_DIR}/tmp/deploy/images/${machine}"
case "${build_id}" in
    *[!0-9]*|'') echo "Build identifier must be a timestamp such as 20260729045937." >&2; exit 2 ;;
esac
artifact="${deploy_dir}/geisa-dev-image-${machine}.rootfs-${build_id}.wic.zst"
[ -f "${artifact}" ] || { echo "No development WIC artifact found for ${build_id}." >&2; exit 1; }

base="${artifact%.wic.zst}"
release_id="$(basename "${base%.rootfs}")"
stage_dir="${platform_root}/release-staging/${release_id}"
[ ! -e "${stage_dir}" ] || { echo "Release destination already exists: ${stage_dir}" >&2; exit 1; }
install -d -m 0755 "${stage_dir}"

for suffix in wic.zst wic.bmap manifest image-manifest.json spdx.json; do
    source="${base}.${suffix}"
    [ -f "${source}" ] || { echo "Missing release input: ${source}" >&2; exit 1; }
    install -m 0644 "${source}" "${stage_dir}/"
done

(
    cd "${stage_dir}"
    sha256sum *.wic.zst *.wic.bmap *.manifest *.image-manifest.json *.spdx.json > SHA256SUMS
)
printf "%s\n" "${stage_dir}"
