...
machine="${GEISA_MACHINE:-${MACHINE_IMX93}}"
artifact=
case "$#" in
    0) ;;
    1)
        case "$1" in
            "${MACHINE_IMX93}"|"${MACHINE_IMX95}") machine="$1" ;;
            *) artifact="$1" ;;
        esac
        ;;
    2) machine="$1"; artifact="$2" ;;
    *) echo "usage: $0 [geisa-imx93|geisa-imx95] [artifact.wic.zst]" >&2; exit 2 ;;
esac
case "${machine}" in
    "${MACHINE_IMX93}"|"${MACHINE_IMX95}") ;;
    *) echo "unsupported machine: ${machine}" >&2; exit 2 ;;
esac
build_dir="${platform_root}/${BUILD_DIR}"
deploy_dir="${build_dir}/tmp/deploy/images/${machine}"
if [ -z "${artifact}" ]; then
    artifact="$(find "${deploy_dir}" -maxdepth 1 -type f -name "geisa-dev-image-${machine}.rootfs-*.wic.zst" -printf "%T@ %p\n" | sort -nr | head -1 | cut -d" " -f2-)"
fi
...
select_e2fsprogs() {
    ...
    if [ -n "${GEISA_E2FSPROGS_BIN_DIR:-}" ]; then
        ...
        candidate="$(find "${build_dir}/tmp/sysroots-components" \
