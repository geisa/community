...
Offline acceptance must also verify that the root filesystem contains the
...
Hardware acceptance must verify that root and `/platform` come from partitions
2 and 3 of the same MMC device, that early platform fsck is clean, and that
Bluetooth controller discovery succeeds without a regulatory-database warning.

The final SD candidate must also confirm the development-only field tools and
kernel capabilities selected for this baseline: `can-utils`, `iw`, `rfkill`,
...
progress. Release artifacts must be built from the reviewed clean source state.
